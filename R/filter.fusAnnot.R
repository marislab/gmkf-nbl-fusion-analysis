...
filter.fusAnnot <- function(caller, red.herring, labels.to.keep){
  ...
  caller.FusAnnot <- unique(caller[grep(red.herring, caller$annots, ignore.case = TRUE),])
  ...
  # arriba.FusAnnot <- unique(ar[grep(red.herring, ar$annots, ignore.case = TRUE),])
  ...
  # sf.FusAnnot <- unique(sf[grep(red.herring, sf$annots, ignore.case = TRUE),])
